...
public class MainActivity extends AppCompatActivity implements
    ...
    protected void onResume() {
        ...
        // DO (1) Check if you are on Android M or later, if so...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            // DO (2) Get a BatteryManager instance using getSystemService()
            BatteryManager battMgr = (BatteryManager)getSystemService(BATTERY_SERVICE);
            // DO (3) Call isCharging on the battery manager and pass the result on to your show
            // charging method
            showCharging(battMgr.isCharging());
        } else {

            // DO (4) If your user is not on M+, then...
            // DO (5) Create a new intent filter with the action ACTION_BATTERY_CHANGED. This is a
            // sticky broadcast that contains a lot of information about the battery state.
            IntentFilter filter = new IntentFilter();
            filter.addAction(Intent.ACTION_BATTERY_CHANGED);
            // DO (6) Set a new Intent object equal to what is returned by registerReceiver, passing in null
            ...
            Intent intentStatus = registerReceiver(null, filter);
            // DO (7) Get the integer extra BatteryManager.EXTRA_STATUS. Check if it matches
            int chargeStatus = intentStatus.getIntExtra(
                    BatteryManager.EXTRA_STATUS, BatteryManager.BATTERY_STATUS_UNKNOWN);
            boolean charging = chargeStatus == BatteryManager.BATTERY_STATUS_CHARGING
            ...
            // DO (8) Update the UI using your showCharging method
